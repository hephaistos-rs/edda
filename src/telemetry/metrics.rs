...
pub fn install(meter: &Meter) {
    ...
    let git_operation_duration = meter
        ...
        .build();
    let _ = INSTRUMENTS.set(Instruments { http_request_duration, git_operation_duration });
}
...
pub fn record_http_request(method: &str, route: &str, status: u16, duration: Duration) {
    let Some(instruments) = INSTRUMENTS.get() else { return };
...
pub fn record_git_operation(operation: &'static str, status: &'static str, duration: Duration) {
    ...
    instruments
        ...
        .record(duration.as_secs_f64() * 1000.0, &[KeyValue::new("operation", operation), KeyValue::new("status", status)]);
}
