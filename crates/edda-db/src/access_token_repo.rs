...
impl AccessTokenRepo {
    pub async fn insert(
        ...
        let row = sqlx::query!(
            "INSERT INTO access_tokens (id, user_id, name, token_hash, repository_scope) VALUES (?, ?, ?, ?, ?) RETURNING created_at",
            id_text,
            user_id_text,
            name,
            token_hash,
    ...
    pub async fn find_by_hash(
        ...
        let _ = sqlx::query!(
            "UPDATE access_tokens SET last_used_at = unixepoch() WHERE token_hash = ?",
            token_hash
        )
        .execute(pool)
        .await;
