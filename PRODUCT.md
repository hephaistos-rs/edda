...
- Codebase is an implemented Cargo workspace: `edda-domain`, `edda-db`, `edda-git`, `edda-render`, `edda-auth`, `edda-jobs`, `edda-app`, `edda-ssh`, `edda-telemetry`, `edda-cli`, and the `edda-web` composition root (Dioxus UI: `layouts`, `pages`, `components`).
