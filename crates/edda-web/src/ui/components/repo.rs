...
use dioxus_free_icons::icons::ld_icons::{
    LdCircleArrowUp, LdCircleCheck, LdCircleDashed, LdLock, LdPlus, LdTriangleAlert,
};
...
pub fn RepoRow(repo: Repo) -> Element {
    let (status_color_class, status_icon) = match repo.status {
        RepoStatus::Clean => (
            "text-status-clean",
            rsx! { Icon { icon: LdCircleCheck, width: 16, height: 16 } },
        ),
        RepoStatus::Ahead => (
            "text-status-ahead",
            rsx! { Icon { icon: LdCircleArrowUp, width: 16, height: 16 } },
        ),
        RepoStatus::Conflict => (
            "text-status-conflict",
            rsx! { Icon { icon: LdTriangleAlert, width: 16, height: 16 } },
        ),
        RepoStatus::Empty => (
            "text-status-empty",
            rsx! { Icon { icon: LdCircleDashed, width: 16, height: 16 } },
        ),
    };
