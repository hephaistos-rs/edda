...
pub fn build_pack(repo: &gix::Repository, wants: &[ObjectId]) -> Result<Vec<u8>, GitError> {
    ...
    while let Some(id) = queue.pop_front() {
        ...
        let object = repo.find_object(id).map_err(|err| GitError::Git(err.to_string()))?;
...
fn serialize_pack(objects: &[(Kind, Vec<u8>)]) -> Result<Vec<u8>, GitError> {
    ...
    hasher.update(&out);
    let checksum = hasher.try_finalize().map_err(|err| GitError::Git(err.to_string()))?;
...
fn kind_from_code(code: u8) -> Result<Kind, GitError> {
    match code {
        ...
        3 => Ok(Kind::Blob),
        4 => Ok(Kind::Tag),
        other => Err(GitError::Git(format!("unsupported pack object type {other}"))),
    }
}
...
fn object_id(kind: Kind, data: &[u8]) -> Result<ObjectId, GitError> {
    ...
    hasher.update(data);
    hasher.try_finalize().map_err(|err| GitError::Git(err.to_string()))
}
...
pub fn parse_pack(repo: &gix::Repository, data: &[u8]) -> Result<Vec<ParsedObject>, GitError> {
        ...
        let (kind, resolved_data) = match type_code {
            ...
            6 => {
                ...
                let base_offset = entry_offset
                    ...
                    .ok_or_else(|| GitError::Git("OFS_DELTA offset points before the pack start".to_string()))?;
            ...
            7 => {
                ...
                let (base_kind, base_data) = match resolved.iter().find(|(_, id, ..)| *id == base_id) {
                    Some((_, _, kind, data)) => (*kind, data.clone()),
                    None => {
                        let object =
                            repo.find_object(base_id).map_err(|err| GitError::Git(err.to_string()))?;
                        (object.kind, object.data.clone())
                    }
                };
                (base_kind, apply_delta(&base_data, &delta)?)
            }
            other => return Err(GitError::Git(format!("unsupported pack entry type {other}"))),
        };
    ...
    Ok(resolved.into_iter().map(|(_, id, kind, data)| ParsedObject { id, kind, data }).collect())
}
...
fn read_object_header(data: &[u8]) -> Result<(u8, usize, usize), GitError> {
    if data.is_empty() {
        return Err(GitError::Git("truncated pack: expected an object header".to_string()));
    }
    let mut pos = 0;
    let first = data[pos];
    pos += 1;
...
fn read_offset_delta(data: &[u8]) -> Result<(usize, usize), GitError> {
    if data.is_empty() {
        return Err(GitError::Git("truncated pack: OFS_DELTA offset".to_string()));
    }
    let mut pos = 0;
...
fn inflate(data: &[u8], expected_size: usize) -> Result<(Vec<u8>, usize), GitError> {
    ...
    if status != Status::StreamEnd || output.len() != expected_size {
        return Err(GitError::Git("zlib stream did not decode to the expected size".to_string()));
    }
...
fn apply_delta(base: &[u8], delta: &[u8]) -> Result<Vec<u8>, GitError> {
    ...
    while pos < delta.len() {
        ...
        if op & 0x80 != 0 {
            ...
            let end = offset.checked_add(size).ok_or_else(|| GitError::Git("delta copy overflow".to_string()))?;
            let chunk = base.get(offset..end).ok_or_else(|| GitError::Git("delta copy out of range".to_string()))?;
...
fn read_delta_size(data: &[u8]) -> Result<(usize, usize), GitError> {
    ...
    let mut shift = 0;
    loop {
        let byte = *data.get(pos).ok_or_else(|| GitError::Git("truncated delta size".to_string()))?;
        pos += 1;
        size |= ((byte & 0x7f) as usize) << shift;
        shift += 7;
        if byte & 0x80 == 0 {
