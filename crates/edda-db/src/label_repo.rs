...
fn row_to_label(row: sqlx::any::AnyRow) -> Result<Label, sqlx::Error> {
    Ok(Label {
...
impl LabelRepo {
    pub async fn insert(
        ...
        match result {
            ...
            Err(sqlx::Error::Database(db_err)) if db_err.is_unique_violation() => {
                Err(InsertLabelError::AlreadyExists(name.to_string()))
            }
    ...
    /// same-scope labels at once, even momentarily.
    pub async fn apply_to_issue(
        pool: &DbPool,
        issue_id: IssueId,
        label: &Label,
    ) -> Result<(), sqlx::Error> {
