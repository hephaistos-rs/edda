...
impl Config {
    pub fn from_env() -> Self {
        let default_service_name = if env_is_set("OTEL_SERVICE_NAME") { None } else { Some("edda") };

        let service_version =
            env::var("OTEL_SERVICE_VERSION").ok().filter(|v| !v.is_empty()).unwrap_or_else(|| env!("CARGO_PKG_VERSION").to_string());
        ...
        let sdk_disabled = env::var("OTEL_SDK_DISABLED").map(|v| v.eq_ignore_ascii_case("true")).unwrap_or(false);

        Self { default_service_name, service_version, default_environment, otlp_configured, sdk_disabled }
    }
    ...
    pub fn sampler(&self) -> Sampler {
        let arg = || env::var("OTEL_TRACES_SAMPLER_ARG").ok().and_then(|v| v.parse::<f64>().ok()).unwrap_or(1.0);
        ...
        match env::var("OTEL_TRACES_SAMPLER").ok().as_deref() {
            ...
            Some("parentbased_traceidratio") => Sampler::ParentBased(Box::new(Sampler::TraceIdRatioBased(arg()))),
