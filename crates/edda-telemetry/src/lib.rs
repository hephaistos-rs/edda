...
impl TelemetryGuard {
    ...
    pub async fn shutdown(self) {
        ...
        async fn shutdown_one<P, E>(
            name: &'static str,
            provider: Option<P>,
            shutdown: impl FnOnce(P) -> Result<(), E> + Send + 'static,
        ) where
            P: Send + 'static,
            E: std::fmt::Display + Send + 'static,
        {
            ...
            match tokio::time::timeout(
                SHUTDOWN_TIMEOUT,
                tokio::task::spawn_blocking(move || shutdown(provider)),
            )
            .await
            {
                Ok(Ok(Ok(()))) => {}
                Ok(Ok(Err(err))) => {
                    tracing::warn!(provider = name, error = %err, "exporter shutdown reported an error")
                }
                Ok(Err(_)) => tracing::warn!(provider = name, "exporter shutdown task panicked"),
                Err(_) => {
                    tracing::warn!(provider = name, timeout = ?SHUTDOWN_TIMEOUT, "exporter shutdown timed out")
                }
            }
        }
...
pub fn init() -> TelemetryGuard {
    ...
    if config.otel_enabled() {
        ...
        let env_filter_fallback = EnvFilter::builder()
            .with_default_directive(default_level.into())
            .from_env_lossy();
        let fallback = tracing_subscriber::registry()
            .with(env_filter_fallback)
